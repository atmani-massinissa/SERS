...
public class Analyseur {
	...
	private boolean unique(String type, String term1, String patron, String term2, String contexte) {
		...
		for (ArrayList<String> Set : Relation.typePatrons.values()) {
			for (String pattern : Set) {
				...
				if (Arrays.asList(new String[] {"ne","celui","ceux","pour","se","ou","qu'elles","à","moins","qu'un","qu'une","que","ce","au","aux","sur","le","en","qu'il", "qu'elle","ceci","plus","sous","très","lui","de","un","être", "est", "qui","elle","celle-ci","il","ils","elles","celui-ci","la","qu'il","y","et","autre",",","s'","l'","ce_qui","au_moins","être"}).contains(term1.toLowerCase()) || 
					Arrays.asList(new String[] {"ne","celui","ceux","pour","se","ou","qu'elles","à","moins","qu'un","qu'une","que","ce","au","aux","sur","le","en","qu'il", "qu'elle","ceci","plus","sous","très","lui","de","un","être", "est", "qui","elle","celle-ci","il","ils","elles","celui-ci","la","qu'il","y","et","autre",",","s'","l'","ce_qui","au_moins","être"}).contains(term2.toLowerCase())) {
					return false;
				}
			}
		}
	...
	public void pretraitementParLemMc() throws Exception{
		...
		//System.out.println("Parser + Lemmatisation + Mots Composés : "+mc.newText);
		this.text = new String(mc.newText);
		//this.text=this.text.replaceAll("[.|;|,|==|\\n|?|!|:|(|)|\\[|\\]|«|»|“|”]", "");
	}
	...
	public void displayResults() throws IOException{
		...
		for (Relation relation : this.getRelations_trouvees()) {
			...
			//System.out.println(" contexte d e la phrase "+relation.getContexte());
		}
	}
}
